...
const DEFAULT_GRPC_ADDR: &str = "localhost:50051";
...
async fn main() {
    ...
    let mut reg_user = pod::RegisteredUser::default();
    loop {
        let cli = Cli::parse();
        match cli.command {
            Commands::Exit => {
            ...
            Commands::Register { username, email, pass } => {
                ...
                reg_user = res.unwrap_or_default();
            }
            ...
            Commands::Create { title, content } => {
                let res =
                if cli.grpc {
                    grpc_client.create_post(&reg_user.token, title, content).await
                }else{
                    let create_req = pod::NewPost{
                        title,
                        content,
                    };
                    http_client.create_post(&reg_user.token, create_req).await
                };
            ...
            Commands::Update { id, title, content } => {
                let res =
                if cli.grpc {
                    grpc_client.update_post(&reg_user.token, id, title, content).await
                }else{
                    let update_req = pod::UpdatePost{
                        title,
                        content,
                    };
                    http_client.update_post(&reg_user.token, pod::PostId{id}, update_req).await
                };
            ...
            Commands::Delete { id} => {
                let res =
                if cli.grpc {
                    grpc_client.delete_post(&reg_user.token, id).await
                }else{
                    http_client.delete_post(&reg_user.token, pod::PostId{id}).await
                };
